$(document).ready(function() {
  ...
  $("form#rsvp_form").submit(function() {
    var this_form = $(this);
    var WDWEDLOCK = Parse.Object.extend("WDWEDLOCK");
    var object = new WDWEDLOCK();
    var data = {
      firstname: this_form[0].first_name.value,
      lastname: this_form[0].last_name.value,
      email: this_form[0].rsvp_email.value,
      invitecode: this_form[0].invite_code.value,
      attend: this_form[0].attending.value === 'yes',
      mealchoice: this_form[0].meal_choice.value
    };
    object.save(data, {
      success: function(object) {
        this_form.fadeOut("fast");
        this_form.parent().find(".success").fadeIn("fast");
        this_form.parent().find(".validation").fadeOut("fast");
      },
      error: function(model, error) {
        this_form.parent().find(".validation").fadeIn("fast");
        this_form.find("input").removeClass("error");
        this_form.find("input").addClass("error");
